...
def seed_link_lr(emb, G1, G2, seed_list1, seed_list2, mul, test_edges_final1, test_edges_final2, alignment_dict,
    ...
    embedding1 = [np.concatenate([np.array(emb[list(G1.nodes()).index(edge[0])]).reshape(64), np.array(emb[list(G1.nodes()).index(edge[1])]).reshape(64),
                                  np.array(emb[list(G1.nodes()).index(edge[0])]).reshape(64) * np.array(emb[list(G1.nodes()).index(edge[1])]).reshape(64)]) for edge
                  in train_edges_G1]
    ...
    embedding2 = [np.concatenate(
        [np.array(emb[list(G2.nodes()).index(edge[0]) + len(G1.nodes())]).reshape(64), np.array(emb[list(G2.nodes()).index(edge[1]) + len(G1.nodes())]).reshape(64),
         np.array(emb[list(G2.nodes()).index(edge[0]) + len(G1.nodes())]).reshape(64) * np.array(emb[
             list(G2.nodes()).index(edge[1]) + len(G1.nodes())]).reshape(64)]) for edge in
                  train_edges_G2]
