...
def structing(layers, G1, G2, G1_degree_dict, G2_degree_dict, attribute, alpha, c):
    ...
    for layer in range(layers + 1):
        ...
        pp_dist_matrix1 = pd.DataFrame(np.transpose(np.array(L1 * G2.number_of_nodes()).reshape(-1, G1.number_of_nodes())),
                          index=G1_nodes, columns=G2_nodes)
        pp_dist_matrix2 = pd.DataFrame(np.array(list(L2 * G1.number_of_nodes())).reshape(-1, G2.number_of_nodes()),
                          index=G1_nodes, columns=G2_nodes)
        pp_dist_df += abs(pp_dist_matrix1 - pp_dist_matrix2)
    for layer in range(layers + 1):
